...
为了让人类更方便理解代码，代码中可以添加`注释` , `注释` 不参与代码表达， 删去注释不影响代码运行和逻辑功能 , 通常是对代码的说明，辅助人们阅读代码，理解代码的深层意义 。
python代码的注释有两种, 一种是单行注释，是以`#`开头的文字行, 还有一种是多行注释是用`'''` 开始 和 结束 的文字  。 例如以下代码示例:
```python
# 这是一个单行注释说明, 删去或添加这种内容不影响代码表达逻辑

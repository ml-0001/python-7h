...
上节课已经将`Python 编程环境`准备完成, 还介绍了一个Python代码的主要结构, 这节课具体展开Python代码的语法和其中的计算表达式部分 。

## 交互式执行代码
由于这节课会有大量短小代码举例, 暂时不用编写长篇代码 , Python的解释器程序提供了一个可以一边输入代码 , 一边执行的 `交互式执行` 环境 , 特别适合验证短小代码语法正确性 。
...
在Python解释器中, 可以直接输入`表达式` , 如果是一个完整的`表达式` 回车后就会输出表达式的结果`值` 。
...
例如:
```python
1+2;(3+5)*7 # 一行有两个语句,通过`;` 分割
2/4 #单行一条语句
1+5+7+7*2+1 #单行一条语句
```

当程序运行时 , 解释器按照输入语句顺序, `从左往右,从上往下`, 顺序执行每条语句 , 这种默认顺序代码执行顺序叫做`顺序执行结构`。 

在你的解释器程序中 , 你可以尝试输入以上代码然后按回车键观察 , 会发现以上每个算数表达式结束后, 解释器都会输出`结果值`, 这是因为如果一条语句是一个`表达式`, 那么每个表达式都有一个结果`值`, 解释器会对表达式`求值`, 为了方便人类观察， 会把求值结果输出到屏幕 。
...
表达式是指, 这个表达式的主要目标是求值 , 通常用来计算或者调用函数执行指定功能 。
...
`常量` 是指在代码中已经明确了(字面)值 , 值**不可改变** 。 比如 `5` , `3.0245`, `"R U OK?"` , 通俗来讲, 这些代码中`写死`的值就叫做(字面)常量 。

> 还记得被遗忘的CE么 ? 你给CE 100元, 跟CE说:"去买`2.5`Kg大米, 再买`1` 个西瓜, `剩余的钱`买糖, 跟老板复述说:`只要麒麟瓜,6斤以上`..." ， 这里的2.5, 1, "只要麒麟瓜,6斤以上" 是你在指令中直接表明的量就是常量 。

对于数字来说, 叫做`数值常量`, 对于文字来说, 叫做`字符串常量` .

`变量` 是指在代码中以`标识符名字` (`变量名`) 代替具体的值 , 而它的值随着代码**可改变** 。
...
代码中的`words` 就是一个变量 , words在作为实参传给print函数时 , print要输出具体的内容(当然不是显示成"words"), 所以需要先对words求值, 只有求值后print才能输出words真正内容"Hi!", 而且随着代码执行 words 的值后面还会被改变为"Hello" 。 同理, 代码中的`x`,`y`也是变量, `x`,`y`, `words` 都只是个具体值的`名字` , 它们的值在运行过程中都可以发生改变 。
...
而`复合表达式` 是指由各种`表达式`通过`运算`构成的表达式 。 这个表达式的定义使用了表达式自身的概念, 这种定义叫做`递归定义`的概念, 有点像数学归纳法吧 ?
就是说表达式可以是一常量或者变量这样的表达式基本单元构成, 还可以是表达式之间通过运算连接构成的表达式 。

按照运算形式不同, 我们可以将复合表达式分为:
* `符号运算表达式`
* `函数运算表达式`

符号运算表达式是指那些用 `运算符` 连接起来的表达式， 例如: `5+4` ， `5 * x * x` , `the_cat_is_smelly or (5 -3) > 0 and (2.0 * x * x) < 2.452` 。

运算符表达式, 一会儿我们会详细介绍 。

函数运算表达式是指, 使用`函数类`参与运算的表达式例如: `math.sin(2.6423)` , `math.sqrt(5.21+209)`, `print("HelloKity")` 。
...
形式是如: `变量名` = `表达式` ， 这就是一条赋值语句 。
执行后 , 这个变量就保存了`表达式的值`, 后续代码就可以通过`变量名` 直接使用值 。
除了`=`还有其他赋值运算符, 如 `+=` , `*=` , `/=` 这些赋值语句和 `=` 的赋值语句类似 , 只不过同时变量还参与了运算, 后文有提及 。
...
代码默认只能顺序执行，但是代码需要处理 `不同条件下`的情况 , 比如假设 `变量` x 表示的是语种, 如果x取值`ZH`时, 输出"NiHao", 否则输出"Hello", 就是代码执行要根据条件选择选择执行 。
那么就得用条件语句来表达 ，在下节课我们详细展开 。

#### 循环语句

代码默认只能顺序执行，但是代码需要处理 `支持重复执行多次` 的情况 , 比如要将 "NiHao" 这段文字输出n次, 而n是一个`变量` (n可能是1, 也可能是10000000) , 那么就得用循环语句来表达 ，在下节课我们详细展开 。

#### 函数定义语句,类定义语句

函数定义是指将完成一定功能的代码封装成一个接口(名字),  然后就可以被其他代码直接用名字使用 。类定义也是类似函数, 就算没有类, 也不影响你的代码描述 , 不过有类之后 , 在语言表达方式上, 比较适合人类的一些思维习惯 。
函数和类在第五课我们详细解释 。
...
> `!表达式` 要么是 `@常量`或者`@变量`, 要么是对 `!表达式` 的 `复合运算` .

其中按`复合运算`语法形式分为:
* `数学符号运算表达式`
* `函数运算表达式`

函数运算是指有函数参与的运算, 这个部分第五课会详细介绍 , 这里详细介绍下数学符号运算的部分 。

Python 支持的运算符有很多, 包括数学上常见到的基础四则运算和逻辑连接等 。 在Python 中的运算符可以分为以下几类 :
...
`逻辑运算表达式`是指运算结果为 `True` (真值) 或者  `False` (假值) 的表达式 , 对应于数学中的`布尔逻辑表达式` , 逻辑表达式往往会有关系判断运算 。
例如在某程序中, 要判断  `番茄价格` 比 `大米价格` 贵  , 就对应到单价数值上就是判断两个数值大小 `tomato_price > rice_price` 。
...
用于描述对多个逻辑表达式连接的情况使用逻辑运算符连接 , 如 `番茄价格` 价格 和  `大米价格` 都比 `西瓜价格` 便宜 , 翻译过来就是说 `watermelon_price > tomato_price`  `并且`  `watermelon_price > rice_price` 。
...
计算机中数值是使用二进制比特位(类似101001001) 编码表示 ，某些情况下为了使用这个二进制表示的特性, 需要操作对应的比特位 , 所以针对整数也提供了位运算操作 。
...
* `~` ,按位取反运算符：对数据的每个二进制位取反,即把1变为0,把0变为1 。~x 类似于 -x-1，在有符号二进制数的补码形式。
...
在数学上，我们知道算数运算+-*/只能针对的是实数运算,  在代码中还有文字, 那么一段文字"R U OK?"和 5做加法是比较奇怪的吧, 语义上是说不通的 , 所以每个运算符对运算对象是有限制的, 所 以需要对运算对象分类 。 
...
不过需要说明 **对于`complex` 复数类型, 向下转换类型是不允许的** 因为复数和整数的转换在数学上不好定义 (建议您尝试写一个将复数转换为整数的表达式 再解释器中观察下)。
